    ...
    <template #config>
      <div class="tencent-config">
        <a-card size="small" :bordered="true" :style="{borderRadius: '8px'}">
          <div class="form-rows">
            <div class="form-row">
              <span class="form-label">SecretId</span>
              <a-input v-model:value="form.secretId" placeholder="请输入 SecretId" @change="save"/>
            </div>
            <div class="form-row">
              <span class="form-label">SecretKey</span>
              <a-input-password v-model:value="form.secretKey" placeholder="请输入 SecretKey" @change="save"/>
            </div>
          </div>
        </a-card>
        ...
        <div class="config-hint">
          <a-typography-link href="https://console.cloud.tencent.com/tapi" target="_blank">
            <LinkOutlined/> 腾讯机器翻译
          </a-typography-link>
        </div>
      </div>
...
import {fetchTencentTranslate} from './tencent-api.js'
...
const form = reactive({
  secretId: props.config.secretId || '',
  secretKey: props.config.secretKey || '',
})

const isConfigured = computed(() => form.secretId && form.secretKey)

watch(() => props.config, (newConfig) => {
  form.secretId = newConfig.secretId || ''
...
async function verify() {
  ...
  try {
    await fetchTencentTranslate('hello', 'en', 'zh', form.secretId, form.secretKey)
...
async function translate(e) {
  ...
  try {
    const result = await fetchTencentTranslate(
      input,
      sourceLang,
      targetLang,
      form.secretId,
    ...
    console.error('[TencentService] translate error:', err)
