    ...
    <template #config>
      <div class="volcano-config">
        <a-card size="small" :bordered="true" :style="{borderRadius: '8px'}">
          <div class="form-rows">
            <div class="form-row">
              ...
              <a-input-password v-model:value="form.accessKeyId" placeholder="请输入 AccessKeyID" @change="save"/>
            </div>
            <div class="form-row">
              <span class="form-label">SecretKey</span>
              <a-input-password v-model:value="form.secretAccessKey" placeholder="请输入 SecretAccessKey" @change="save"/>
            </div>
          </div>
        </a-card>
        ...
        <div class="config-hint">
          <a-typography-link href="https://www.volcengine.com/product/translate" target="_blank">
            <LinkOutlined/> 火山引擎翻译
          </a-typography-link>
        </div>
      </div>
      ...
      <div class="volcano-result">
        ...
        <div v-else-if="!isConfigured" class="placeholder">
          <a-typography-text type="secondary" :style="{fontSize: '13px'}">
            请先配置 AccessKeyID 和 SecretAccessKey
          </a-typography-text>
        </div>
...
import {fetchVolcanoTranslate} from './volcano-api.js'
...
const isConfigured = computed(() => form.accessKeyId && form.secretAccessKey)

watch(() => props.config, (newConfig) => {
  form.accessKeyId = newConfig.accessKeyId || ''
  form.secretAccessKey = newConfig.secretAccessKey || ''
}, {immediate: true})
...
async function verify() {
  ...
  try {
    await fetchVolcanoTranslate('hello', 'en', 'zh', form.accessKeyId, form.secretAccessKey)
...
async function translate(e) {
  ...
  try {
    const result = await fetchVolcanoTranslate(
      input, sourceLang, targetLang,
      form.accessKeyId, form.secretAccessKey, signal
    )
