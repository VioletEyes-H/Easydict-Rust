    ...
    <template #config>
      <div class="openai-config">
        ...
        <div class="config-hint">
          <a-typography-link href="https://platform.openai.com" target="_blank">
            <LinkOutlined/> OpenAI Platform
          </a-typography-link>
        </div>
      </div>
    </template>
...
import {fetchOpenAITranslate} from './openai-api.js'
...
watch(() => props.config, (newConfig) => {
  ...
  form.model = newConfig.model || 'gpt-4o-mini'
}, {immediate: true})
...
async function verify() {
  ...
  try {
    await fetchOpenAITranslate('hello', 'en', 'zh', form.apiKey, form.endpoint, form.model)
...
async function translate(e) {
  ...
  try {
    const result = await fetchOpenAITranslate(
      input, sourceLang, targetLang,
      form.apiKey, form.endpoint, form.model, signal
    )
    ...
    console.error('[OpenAIService] translate error:', err)
